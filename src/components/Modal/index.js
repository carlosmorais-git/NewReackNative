      ...
      <Pressable style={styles.overlay} onPress={onClose}>
        <Pressable style={styles.card} onPress={() => {}}>
          <View style={[styles.iconWrapper, { backgroundColor: config.iconBg }]}>
            <MaterialCommunityIcons name={config.iconName} size={24} color={config.iconColor} />
          </View>
          <Text style={styles.titulo}>{titulo}</Text>
          <Text style={styles.mensagem}>{mensagem}</Text>
          <View style={styles.botoes}>
            <TouchableOpacity
              style={[styles.btnConfirmar, { backgroundColor: config.btnColor }]}
              onPress={onConfirm}
              activeOpacity={0.85}
            >
              <Text style={styles.btnConfirmarText}>{textoBotaoConfirmar}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.btnCancelar} onPress={onClose} activeOpacity={0.7}>
